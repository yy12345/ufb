...
var showErrorInfo = function (wrap, errText) {
    ...
    // var obj = wrap.find(".tips-check");
    // obj.html("错误提示：" + errText).addClass("formErr").show();
    // obj.parent().show();
    // obj.parent().next().hide();
}
var showErrorInfo2 = function (errText) {
	$(".wrongTips").addClass("wrongTipsErr").show();
    $(".wrongTips").html("<span></span>" + errText + "<b></b>");
}
var showOkInfo2 = function () {
	$(".wrongTips").removeClass("wrongTipsErr").hide();
    $(".wrongTips").html("<span></span><b></b>");
}
...
var showOkInfo = function (wrap) {
    ...
    // var obj = wrap.find(".tips-check");
    // obj.html("").removeClass("formErr").hide();
    // obj.parent().hide();
    // obj.parent().next().show();
}
var showTips = function (ele, text) {
    var _wrap = ele.parents("li");
    // var _wrap = ele.parents("tr.reg-tr"); //TODO
    if(!text) {
        showOkInfo(_wrap);
        // ADD BY GH
        return true;
...
var showTips2 = function (ele, text) {
    if(!text) {
        showOkInfo2();
        return true;
    }else {
...
/* check mod */
var check_null2 = function (ipt, text) {
    var value = ipt.val();
    if (isNull(value)) {
        return showTips2(ipt, text);
    }
    return showTips2(ipt, 0);
}
// TODOEND
/* check mod */
